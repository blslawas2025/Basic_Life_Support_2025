...
async function fixRemainingEmptySectionResults() {
  try {
    console.log('🔍 Fixing remaining empty section_results with proper completed_items...');
    ...
    for (const record of emptyRecords) {
      ...
      const newTotalItems = standardStructure.reduce((total, section) => total + section.items.length, 0);
      const newCompletedItems = Math.min(record.completed_items || 0, newTotalItems); // Don't exceed new total
      const newCompletionPercentage = newTotalItems > 0 ? Math.round((newCompletedItems / newTotalItems) * 100) : 0;
      
      console.log(`   Old: ${record.completed_items}/${record.total_items} (${record.completion_percentage}%)`);
      console.log(`   New: ${newCompletedItems}/${newTotalItems} (${newCompletionPercentage}%)`);
...
fixRemainingEmptySectionResults();
